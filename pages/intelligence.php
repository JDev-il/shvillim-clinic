<!DOCTYPE html>
<html lang="he">

<head>
  <?php include_once "./components/head.php"; ?>
</head>

<body>
  <?php include_once "./components/nav.php"; ?>

  <div class="jumbotron jumbotron-fluid jumboTronArticles z-depth-0" title="Photo by Jonas Jacobsson on Unsplash" alt="Photo by Jonas Jacobsson on Unsplash">
    <h1>אינטיליגנציה רגשית</h1>
    <!-- <p class="articlesPicsignature"></p> -->
  </div>
  <div class="container col-lg-8" id="intelligence">
    <!-- JSON DATA -->
  </div>
  <div class="animate">
    <button type="button" class="btn btn-md btnScrollTop" title="לתחילת הדף">
      <i class="fas fa-angle-double-up"></i>
    </button>
  </div>


  <footer class="triggerFooter">
    <div class="container text-center text-md-right">
      <div class="row rowFooter">
        <div class="col-md-3 mx-auto">
          <ul class="">
            <li class="">
              <a class="" href="../home.html">
                דף הבית
              </a>
            </li>
          </ul>
          <ul>
            <li class="tonyLi">
              <a class="" href="./tonylaviad.html">
                ד"ר טוני לביאד
              </a>
            </li>
          </ul>
          <ul class="">
            <li class="">
              <a class="" href="./privacy.html">
                פרטיות
              </a>
            </li>
          </ul>
        </div>
        <div class="col-md-3 mx-auto">
          <ul class="">
            <li class="">
              <a class="" href="./terms.html">
                תנאי שימוש
              </a>
            </li>
          </ul>
          <ul class="">
            <li class="">
              <a class="" href="./contact.html">
                צור קשר
              </a>
            </li>
          </ul>
        </div>
      </div>
      <div class="justify-content-center tonyRights">
        <p>
          © כל הזכויות שמורות לד"ר טוני לביאד | מכון "שבילים"
        </p>
        <span>עיצוב ופיתוח: WeWebs</span>
        <br />
        <img class="sslBadge" title="RapidSSl Certificated" src="../content/media/RapidSSL_SEAL-90x50.gif" />
      </div>
    </div>
  </footer>
</body>

</html>